	...
	specs := map[string]struct {
		...
		"invalid extension": {
			srcMutator: func(c *ContractInfo) {
				...
				any, err := codectypes.NewAnyWithValue(&oldgovtypes.TextProposal{})
		...
		"not validatable extension": {
			srcMutator: func(c *ContractInfo) {
				// any protobuf type with ValidateBasic method
				any, err := codectypes.NewAnyWithValue(&oldgovtypes.Proposal{})
				require.NoError(t, err)
				c.Extension = any
			},
		},
...
func TestContractInfoSetExtension(t *testing.T) {
	...
	aNestedProtobufExt := func() ContractInfoExtension {
		...
		myExtension, err := oldgovtypes.NewProposal(&oldgovtypes.TextProposal{Title: "bar"}, 1, anyTime, anyTime)
		require.NoError(t, err)
	...
	specs := map[string]struct {
		...
		"validated and accepted": {
			src: &oldgovtypes.TextProposal{Title: "bar", Description: "set"},
		},
		"validated and rejected": {
			src:    &oldgovtypes.TextProposal{Title: "bar"},
			expErr: true,
		},
	}
...
func TestContractInfoMarshalUnmarshal(t *testing.T) {
	...
	oldgovtypes.RegisterInterfaces(interfaceRegistry)
	...
	var destExt oldgovtypes.Proposal
...
func TestContractInfoReadExtension(t *testing.T) {
	anyTime := time.Now().UTC()
	myExtension, err := oldgovtypes.NewProposal(&oldgovtypes.TextProposal{Title: "foo"}, 1, anyTime, anyTime)
	require.NoError(t, err)
	...
	specs := map[string]struct {
		...
		"non matching types": {
			...
			param: func() ContractInfoExtension {
				return &oldgovtypes.TextProposal{}
			},
			expErr: true,
		},
