...
func TestWasmdExport(t *testing.T) {
	db := db.NewMemDB()
	gapp := NewWasmApp(log.MustNewDefaultLogger(log.LogFormatPlain, log.LogLevelDebug, false), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyBaseAppOptions{}, emptyWasmOpts)
	...
	newGapp := NewWasmApp(log.MustNewDefaultLogger(log.LogFormatPlain, log.LogLevelDebug, false), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyBaseAppOptions{}, emptyWasmOpts)
